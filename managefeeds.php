 ...
 * @copyright 2022 Martin CORNU-MANSUY
...
// TODO check why we never get the returnurl.
...
$baseurl = new moodle_url('/blocks/rss_thumbnails/managefeeds.php', $urlparams);
$PAGE->set_url($baseurl);

// Process any actions.
if ($deleterssid && confirm_sesskey()) {
    $DB->delete_records('block_rss_thumbnails', array('id' => $deleterssid));

    redirect($PAGE->url, get_string('feeddeleted', 'block_rss_thumbnails'));
}
...
$feeds = $DB->get_records_select('block_rss_thumbnails', $select, null, $DB->sql_order_by_text('title'));

$strmanage = get_string('managefeeds', 'block_rss_thumbnails');
...
$table->define_headers(array(get_string('feed', 'block_rss_thumbnails'), get_string('actions', 'moodle')));
...
foreach ($feeds as $feed) {
    ...
    $viewlink = html_writer::link(
            $CFG->wwwroot .'/blocks/rss_thumbnails/viewfeed.php?rssid=' . $feed->id . $extraparams, $feedtitle
    );
    ...
    if ($feed->skipuntil) {
        ...
        $skipmsg = get_string('failedfeed', 'block_rss_thumbnails', $skipuntil);
        $notification = new \core\output\notification($skipmsg, 'error');
        $notification->set_show_closebutton(false);
        $feedinfo .= $OUTPUT->render($notification);
    }

    $editurl = new moodle_url('/blocks/rss_thumbnails/editfeed.php?rssid=' . $feed->id . $extraparams);
    $editaction = $OUTPUT->action_icon($editurl, new pix_icon('t/edit', get_string('edit')));

    $deleteurl = new moodle_url(
            '/blocks/rss_thumbnails/managefeeds.php?deleterssid=' . $feed->id . '&sesskey=' . sesskey() . $extraparams
    );
    $deleteicon = new pix_icon('t/delete', get_string('delete'));
    $deleteaction = $OUTPUT->action_icon(
            $deleteurl,
            $deleteicon,
            new confirm_action(get_string('deletefeedconfirm', 'block_rss_thumbnails'))
    );
...
$url = $CFG->wwwroot . '/blocks/rss_thumbnails/editfeed.php?' . substr($extraparams, 1);
